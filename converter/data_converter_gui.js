...
// refer to Module (if they choose; they can also define Module)
  ...
  (function() {
    ...
    var loadPackage = function(metadata) {
      ...
      var PACKAGE_NAME = 'data_converter_gui.data';
      var REMOTE_PACKAGE_BASE = 'data_converter_gui.data';
...
Module['FS_createPath']("/", "assets", true, true);
      ...
      function processPackageData(arrayBuffer) {
        ...
        // Reuse the bytearray from the XHR as the source for file reads.
          ...
          }          Module['removeRunDependency']('datafile_data_converter_gui.data');

      };
      Module['addRunDependency']('datafile_data_converter_gui.data');
    ...
    loadPackage({"files": [{"filename": "/assets/ferndale_studio_11_4k.hdr", "start": 0, "end": 25371653}, {"filename": "/assets/grid.jpg", "start": 25371653, "end": 26171695}, {"filename": "/assets/tc22.glb", "start": 26171695, "end": 51501471}], "remote_package_size": 51501471});
...
var wasmBinaryFile;
  wasmBinaryFile = 'data_converter_gui.wasm';
...
var ASM_CONSTS = {
  462168: ($0) => { const pathPtr = $0; let input = Module.__dataConverterInputFile; if (!input) { input = document.createElement('input'); input.type = 'file'; input.style.display = 'none'; document.body.appendChild(input); Module.__dataConverterInputFile = input; } input.accept = '.cgi,.fbx'; input.onchange = function(event) { const file = event.target.files && event.target.files[0]; if (!file) return; const reader = new FileReader(); reader.onload = function() { const data = new Uint8Array(reader.result); try { FS.mkdir('/uploads'); } catch (e) {} const safeName = file.name.replace(/[^A-Za-z0-9._-]/g, '_'); const virtualPath = '/uploads/' + safeName; FS.writeFile(virtualPath, data); Module.ccall('DataConverterGuiSetDialogPath', null, ['number', 'string'], [pathPtr, virtualPath]); }; reader.readAsArrayBuffer(file); input.value = ""; }; input.click(); },  
 463017: ($0) => { const pathPtr = $0; try { FS.mkdir('/downloads'); } catch (e) {} const name = window.prompt('Output file name', 'output.fbx'); if (!name) return; const safeName = name.replace(/[^A-Za-z0-9._-]/g, '_'); Module.ccall('DataConverterGuiSetDialogPath', null, ['number', 'string'], [pathPtr, '/downloads/' + safeName]); },  
 463335: ($0) => { const path = UTF8ToString($0); if (!FS.analyzePath(path).exists) return; const data = FS.readFile(path); const blob = new Blob([data], { type: 'application/octet-stream' }); const url = URL.createObjectURL(blob); const link = document.createElement('a'); link.href = url; link.download = path.split('/').pop() || 'converted.fbx'; document.body.appendChild(link); link.click(); document.body.removeChild(link); setTimeout(function() { URL.revokeObjectURL(url); }, 1000); },  
 463809: () => { if (document.fullscreenElement) return 1; },  
 463855: () => { return document.getElementById('canvas').width; },  
 463907: () => { return parseInt(document.getElementById('canvas').style.width); },  
 463975: () => { document.exitFullscreen(); },  
 464002: () => { setTimeout(function() { Module.requestFullscreen(false, false); }, 100); },  
 464075: () => { if (document.fullscreenElement) return 1; },  
 464121: () => { return document.getElementById('canvas').width; },  
 464173: () => { return screen.width; },  
 464198: () => { document.exitFullscreen(); },  
 464225: () => { setTimeout(function() { Module.requestFullscreen(false, true); setTimeout(function() { canvas.style.width="unset"; }, 100); }, 100); },  
 464358: () => { return window.innerWidth; },  
 464384: () => { return window.innerHeight; },  
 464411: () => { if (document.fullscreenElement) return 1; },  
 464457: () => { return document.getElementById('canvas').width; },  
 464509: () => { return parseInt(document.getElementById('canvas').style.width); },  
 464577: () => { if (document.fullscreenElement) return 1; },  
 464623: () => { return document.getElementById('canvas').width; },  
 464675: () => { return screen.width; },  
 464700: () => { return window.innerWidth; },  
 464726: () => { return window.innerHeight; },  
 464753: () => { if (document.fullscreenElement) return 1; },  
 464799: () => { return document.getElementById('canvas').width; },  
 464851: () => { return screen.width; },  
 464876: () => { document.exitFullscreen(); },  
 464903: () => { if (document.fullscreenElement) return 1; },  
 464949: () => { return document.getElementById('canvas').width; },  
 465001: () => { return parseInt(document.getElementById('canvas').style.width); },  
 465069: () => { document.exitFullscreen(); },  
 465096: ($0) => { document.getElementById('canvas').style.opacity = $0; },  
 465154: () => { return screen.width; },  
 465179: () => { return screen.height; },  
 465205: () => { return window.screenX; },  
 465232: () => { return window.screenY; },  
 465259: ($0) => { navigator.clipboard.writeText(UTF8ToString($0)); },  
 465312: ($0) => { document.getElementById("canvas").style.cursor = UTF8ToString($0); },  
 465383: () => { document.getElementById('canvas').style.cursor = 'none'; },  
 465440: ($0, $1, $2, $3) => { try { navigator.getGamepads()[$0].vibrationActuator.playEffect('dual-rumble', { startDelay: 0, duration: $3, weakMagnitude: $1, strongMagnitude: $2 }); } catch (e) { try { navigator.getGamepads()[$0].hapticActuators[0].pulse($2, $3); } catch (e) { } } },  
 465696: ($0) => { document.getElementById('canvas').style.cursor = UTF8ToString($0); },  
 465767: () => { if (document.fullscreenElement) return 1; },  
 465813: () => { return window.innerWidth; },  
 465839: () => { return window.innerHeight; },  
 465866: () => { if (document.pointerLockElement) return 1; }
};
...
/** @type {function(...*):?} */
var _DataConverterGuiSetDialogPath = Module["_DataConverterGuiSetDialogPath"] = createExportWrapper("DataConverterGuiSetDialogPath");
/** @type {function(...*):?} */
